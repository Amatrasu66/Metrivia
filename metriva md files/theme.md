...
1. In the scatter chart, when the scatter chart is being shown, there are no vibrations when a user is dragging his finger over the scatter chart because there are various points, and each point must have a vibration.
2. I want to add a settings page to this entire project. In the settings page, a user can go and customize themes for this app according to themselves. If possible, I also want to make the user able to adjust the intensity of the vibrations or any other things individually.

  In the settings page for the themes that I was talking about, in Tweaks CN, there are multiple themes available. I will provide you with the code for the multiple themes that are available in Tweaks CN. There will be a default theme, and if the user does not like the default theme, the user can go to the settings page, change the theme according to what they like, and pick the themes from the settings page.  In the future, we will do something like a user can make their custom theme from the themes selector or something. That is a future thing. For now, below I will give you the codes for the various themes I would like to add. In the settings page, if the user goes and presses that theme, the entire theme of the app will be changed. 
...
And one last thing: for each theme right now above, I have just written "Theme 1", "Theme 2", and "Theme 3". In the settings page that we are going to build, each theme will have a distinct name for it that you can just choose by yourself. And one more thing: I want to add a selector for dark mode and light mode on the settings page too, so that a user can go and switch to dark mode and light mode while changing the theme. 